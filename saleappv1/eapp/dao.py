...
def add_user(name, username, password, avatar):
    if len(username) < 5:
        raise ValueError("ten dang nhap phai toi thieu 5 ki tu!")
    if len(password) < 8:
        raise ValueError("password phai toi thieu 8 ki tu!")
    if not re.search(r'[a-zA-Z]', password):
        raise ValueError('password phai chua li tu!')
    if not re.search(r'[0-9]', password):
        raise ValueError('password phai chua so!')
    if User.query.filter(User.username.__eq__(username)).first():
        raise ValueError(f'Username {username} da ton tai!')
